...
public class MyAnnotator implements Annotator {
  ...
  public void annotate(@NotNull PsiElement psiElement, @NotNull AnnotationHolder annotationHolder) {
    ...
    if (psiElement instanceof PsiClass) {
      ...
      if (name != null && !isUpperCamelCase(name)) {
        warnWithFix(annotationHolder, ((PsiClass) psiElement).getNameIdentifier().getNode(),
                "Basmach Standard: Class names must be UpperCamelCase", "Convert to UpperCamelCase",
                (file) -> {
  ...
  public void checkEmptyLines(AnnotationHolder holder, PsiElement whitespace, PsiElement possibleBrace, String noEmptyLinesMsg, String oneEmptyLineMsg) {
    ...
    if (possibleBrace == null) {
      return;
    }
    
    if (nextNode.getPsi() instanceof PsiIfStatement && nextNode.getTreeParent().getPsi() instanceof PsiIfStatement) {
      return;
    }
  ...
  private boolean isConstant(PsiVariable variable) {
    ...
    if (variable instanceof PsiField && "serialVersionUID".equals(variable.getName())) {
      return false;
    }
  ...
  private static final String[] NAMES_TO_TWENTY = {"Zero", "One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine", "Ten",
          "Eleven", "Twelve", "Thirteen", "Fourteen", "Fifteen", "Sixteen", "Seventeen", "Eighteen", "Nineteen"};
  
  private static final String[] TEN_MULTIPLE_NAMES = {"Zero", "Ten", "Twenty", "Thirty", "Forty", "Fifty", "Sixty", "Seventy", "Eighty", "Ninety"};
  ...
  private String nameNumber(int number, boolean zeroBlank) {
    ...
    if (number < 20) {
      return NAMES_TO_TWENTY[number];
    } else if (number < 100) {
      return TEN_MULTIPLE_NAMES[number / 10] + nameNumber(number % 10, true);
    } else if (number < 1000) {
      int h = number / 100;
      
      return NAMES_TO_TWENTY[h] + "Hundred" + nameNumber(number % 100, true);
    } else if (number < 10000) {
      int t = number / 1000;
      
      return NAMES_TO_TWENTY[t] + "Thousand" + nameNumber(number % 1000, true);
    }
