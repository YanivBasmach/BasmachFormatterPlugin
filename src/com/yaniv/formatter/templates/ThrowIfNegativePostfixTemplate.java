...
public class ThrowIfNegativePostfixTemplate extends JavaEditablePostfixTemplate {
  public ThrowIfNegativePostfixTemplate(@NotNull PostfixTemplateProvider provider) {
    super("thrifneg", "if ($EXPR$ < 0) { throw new $EXCEPTION$($END$); }", "if (myVar < 0) { throw new InvalidParameterException(\"The provided value must not be negative!\"); }", ImmutableSet.of(new JavaPostfixTemplateExpressionCondition.JavaPostfixTemplateNumberExpressionCondition()), LanguageLevel.JDK_1_3, false, provider);
  }
